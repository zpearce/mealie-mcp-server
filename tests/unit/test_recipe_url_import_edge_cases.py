...
@pytest.mark.asyncio
async def test_import_recipe_with_unusual_formats(test_env, httpx_mock):
    """Test importing recipes from various URL formats."""
    mcp_server = create_test_server(httpx_mock)
    ...
    httpx_mock.add_response(
        ...
        url="http://test.mealie.local/api/recipes/create/url",
        status_code=400,
        json={"detail": "Cannot extract recipe from https://youtube.com/watch?v=abc123"},
    )
    httpx_mock.add_response(
        method="POST",
        url="http://test.mealie.local/api/recipes/create/url",
        status_code=400,
        json={"detail": "Cannot extract recipe from https://instagram.com/p/abc123/"},
    )
    httpx_mock.add_response(
        method="POST",
        url="http://test.mealie.local/api/recipes/create/url",
        status_code=400,
        json={"detail": "Cannot extract recipe from https://example.com/recipe.pdf"},
    )
    ...
    test_urls = [
        ...
        "ftp://example.com/recipes/file.txt",
    ]

    for test_url in test_urls:
        if test_url.startswith("ftp://"):
            async with Client(mcp_server) as client:
                result = await client.call_tool(
                    "import_recipe_from_url",
                    {"url": test_url}
